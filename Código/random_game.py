...
def change_color(pos):
    global current_circle, current_green_circle, mouse_positions, mouse_lists

    # Check if click is on the blue circle
    if point_inside_circle(pos, (central_x, central_y), central_radius):
        # Save mouse positions for the movement
        if mouse_positions:
            mouse_lists.append(np.array(mouse_positions))
            mouse_positions = []
        # Generate a new green circle
        current_green_circle = random.randint(0, len(circles) - 1)
        circles[current_green_circle]["color"] = GREEN
        return
    ...
    for key, circle in circles.items():
        if point_inside_circle(pos, circle["center"], circle_radius):
            # If the circle is green, turn it off (change to red)
            if key == current_green_circle:
                circle["color"] = RED
                current_green_circle = None
                if mouse_positions:
                    mouse_lists.append(np.array(mouse_positions))
                    mouse_positions = []
                return
            # If a circle is already selected, change it back to red
            if current_circle is not None and key != current_green_circle:
            ...
            current_circle = key
            # Save mouse positions for the movement
            mouse_positions.append(circle["center"])
            return
...
# Choose a random red circle to be green initially
current_green_circle = random.randint(0, len(circles) - 1)
circles[current_green_circle]["color"] = GREEN
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_c:
                running = False  # Quit the game without saving
                break
...
if mouse_lists:
    with open("mouse_positions.txt", "w") as file:
        for mouse_list in mouse_lists:
            for position in mouse_list:
